...
async function boot(){
  try{
    ...
    $('categoryTree').innerHTML=`<div class="db-error">${esc(e.message)}<br>請用 START_V0.6_POSTGRES_PRODUCTS.bat 開啟，不要從 GitHub Pages 測試。</div>`;
  }
}
async function loadProducts(q=''){
  $('categoryTree').innerHTML='<div class="empty">正在辨識 PostgreSQL 真實產品與品號...</div>';
...
function renderRows(id,rows,keys){$(id).innerHTML=rows.length?rows.map(r=>`<tr>${keys.map(k=>`<td>${esc(r[k])}</td>`).join('')}</tr>`).join(''):`<tr><td colspan="${keys.length}" class="empty">V0.6 尚未接此關聯表</td></tr>`;}
...
$('searchBtn').onclick=search;$('searchInput').addEventListener('keydown',e=>{if(e.key==='Enter')search();});$('labelSearch').addEventListener('input',renderLabels);['batchNo','printQty','packQty','labelDate','labelSpec','labelSize'].forEach(id=>$(id).addEventListener('input',refreshPreview));$('printBtn').onclick=printLabels;$('reloadBtn').onclick=()=>loadProducts($('searchInput').value.trim());$('copyBtn').onclick=()=>alert('V0.6 是 PostgreSQL 唯讀測試版，尚未開放複製或寫入。');$('labelDate').value=new Date().toISOString().slice(0,10);boot();
