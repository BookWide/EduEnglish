...
const CONFIG={
  ...
  PMK_API: localStorage.getItem("pmktools_product_api") || "https://pmktools-api.pmktools.workers.dev"
};
...
async function pmkApi(path,opt={}){
  const headers={"Content-Type":"application/json",...(opt.headers||{})};
  const r=await fetch(CONFIG.PMK_API+path,{cache:"no-store",...opt,headers});
  const j=await r.json().catch(()=>({}));
  if(!r.ok||j.ok===false)throw new Error(j.message||j.error||`HTTP ${r.status}`);
  return j;
}
function isIndustrialProduct(p){
  const text=[p?.category,p?.name,p?.brand,p?.source_type,p?.fulfillment_type,p?.sale_mode]
    .map(x=>String(x||"").toLowerCase()).join(" ");
  if(/切削|刀具|刀片|刀桿|刀杆|高壓|高压|冷卻|冷却|工業|工业|tool|cutting|coolant/.test(text)) return true;
  if(String(p?.fulfillment_type||"").toLowerCase()==="shipping") return true;
  if(String(p?.sale_mode||"").toLowerCase()==="cash") return true;
  const vars=Array.isArray(p?.variations)?p.variations:[];
  return vars.some(v=>{
    const sku=String(v?.sku||v?.source_sku_id||"").trim().toUpperCase();
    return sku && !sku.startsWith("BW-");
  });
}
function industrialCategory(p){
  const text=[p?.category,p?.name].map(x=>String(x||"")).join(" ");
  if(/高壓|高压|冷卻|冷却/.test(text)) return "高壓冷卻系統";
  if(/切削|刀具|刀片|刀桿|刀杆|槽刀|車刀|铣刀|銑刀/.test(text)) return "切削刀具";
  return String(p?.category||"其他工業品").trim()||"其他工業品";
}
function normalizePmkIndustrial(products){
  const out=[];
  (products||[]).filter(p=>p&&p.is_active!==false&&isIndustrialProduct(p)).forEach(p=>{
    const vars=Array.isArray(p.variations)?p.variations.filter(v=>v&&v.is_active!==false):[];
    if(vars.length){
      vars.forEach((v,i)=>{
        const sku=String(v.sku||v.source_sku_id||"").trim();
        if(!sku)return;
        const spec=String(v.variation_name||Object.values(v.attributes||{}).join(" / ")||"").trim();
        out.push({
          sku,name:String(p.name||"工業品"),description:spec,category:industrialCategory(p),
          brand:String(p.brand||""),unit:"PCS",price:Number(v.sale_price||v.price||p.price||0),
          source_sku_id:String(v.source_sku_id||""),stock_qty:Number(v.stock_qty||0),
          status:v.is_active===false?"INACTIVE":"ACTIVE",image_url:String(v.image_url||p.image_url||""),
          source_group:"industrial",source_product_id:String(p.id||p.external_id||""),
          source_variation_id:String(v.id||`var-${i+1}`)
        });
      });
    }else{
      const sku=String(p.sku||p.sku_prefix||p.external_id||p.id||"").trim();
      if(!sku)return;
      out.push({
        sku,name:String(p.name||"工業品"),description:String(p.detail_summary||p.detail_description||""),
        category:industrialCategory(p),brand:String(p.brand||""),unit:"PCS",
        price:Number(p.sale_price||p.price||0),source_sku_id:String(p.source_sku_id||""),
        stock_qty:Number(p.stock_qty||0),status:p.is_active===false?"INACTIVE":"ACTIVE",
        image_url:String(p.image_url||""),source_group:"industrial",source_product_id:String(p.id||p.external_id||"")
      });
    }
  });
  return out;
}
function normalizeGiftItems(rows){
  return (rows||[]).map(x=>({...x,source_group:"gift",category:String(x.category||"精選福利"),unit:x.unit||"PCS"}));
}
...
const itemsModule={
 ...
 async render(page,ctx){
   if(page==="inventory"){
     ...
     ctx.ws.innerHTML=`<div class="card"><h2>庫存總覽</h2><table class="gridTable"><tr><th>SKU</th><th>名稱</th><th>目前庫存</th><th>安全存量</th></tr>${(j.rows||[]).map(x=>`<tr><td>${esc(x.sku)}</td><td>${esc(x.name)}</td><td>${money(x.stock_qty)}</td><td>${money(x.safety_stock)}</td></tr>`).join("")}</table></div>`;
     return
   }

   const [giftRes,pmkRes]=await Promise.allSettled([api("/api/items"),pmkApi("/api/pmk-erp-items")]);
   const giftRows=giftRes.status==="fulfilled"?normalizeGiftItems(giftRes.value.rows||[]):[];
   const pmkPayload=pmkRes.status==="fulfilled"?pmkRes.value:{};
   const industrialRows=(pmkPayload.rows||[]).map(x=>({...x,source_group:"industrial"}));
   const rows=[...industrialRows,...giftRows];

   ctx.ws.innerHTML=`<div class="layout2">
     <aside class="listPanel">
       <b>Items - 商品總覽</b>
       <input id="iq" class="listSearch" placeholder="SKU / 名稱">
       <div class="sourceSummary">
         <span class="srcBadge industrial">PMKTOOLS 工業品 ${industrialRows.length}</span>
         <span class="srcBadge gift">BookWide Gift ${giftRows.length}</span>
       </div>
       <div id="ilist"></div>
     </aside>
     <section>
       <div class="tabs"><button class="active">基本資料</button><button>其他資料</button><button>交易對象</button><button>安全存量</button><button>庫存數量</button><button>附件</button><button>狀態</button></div>
       <div id="itemForm"></div>
     </section>
   </div>`;

   const grouped=data=>{
     const industrial=new Map(),gifts=new Map();
     data.forEach(x=>{
       const target=x.source_group==="industrial"?industrial:gifts;
       const cat=String(x.category||(x.source_group==="industrial"?"其他工業品":"精選福利"));
       if(!target.has(cat))target.set(cat,[]);
       target.get(cat).push(x);
     });
     return {industrial,gifts};
   };
   const groupHtml=(title,kind,map)=>{
     const total=[...map.values()].reduce((n,a)=>n+a.length,0);
     return `<div class="itemMajor ${kind}">
       <div class="itemMajorHead">📁 ${esc(title)} <small>(${total})</small></div>
       ${[...map.entries()].map(([cat,items])=>`<div class="itemCategory">
         <div class="itemCategoryHead">▾ ${esc(cat)} <small>(${items.length})</small></div>
         ${items.sort((a,b)=>String(a.sku).localeCompare(String(b.sku))).map(x=>`<button class="rowBtn ${x.source_group}" data-key="${esc(x.source_group+"::"+x.sku)}"><b>${esc(x.sku)}</b><span>${esc(x.name)}</span>${x.description?`<small>${esc(x.description)}</small>`:""}</button>`).join("")}
       </div>`).join("")}
     </div>`;
   };
   const show=x=>{
     if(!x)return;
     const source=x.source_group==="industrial"?"PMKTOOLS 工業品":"BookWide Gift / LINE 福利品";
     document.querySelector("#itemForm").innerHTML=`<div class="itemSource ${x.source_group}">${esc(source)}</div><div class="formGrid">
       <label>編號 / SKU<input value="${esc(x.sku)}" readonly></label>
       <label class="span2">名稱<input value="${esc(x.name)}" readonly></label>
       <label>狀態<input value="${esc(x.status||"ACTIVE")}" readonly></label>
       <label class="span4">說明<textarea readonly>${esc(x.description||"")}</textarea></label>
       <label>大項<input value="${esc(source)}" readonly></label>
       <label>分類<input value="${esc(x.category||"")}" readonly></label>
       <label>品牌<input value="${esc(x.brand||"")}" readonly></label>
       <label>單位<input value="${esc(x.unit||"PCS")}" readonly></label>
       <label>前台牌價<input value="${money(x.price)}" readonly></label>
       <label>來源 SKU<input value="${esc(x.source_sku_id||"")}" readonly></label>
       <label>庫存<input value="${money(x.stock_qty)}" readonly></label>
       <label>來源商品 ID<input value="${esc(x.source_product_id||"")}" readonly></label>
       ${x.image_url?`<label class="span4">商品圖片<div class="erpItemImageWrap"><img src="${esc(x.image_url)}" alt=""></div></label>`:""}
     </div>`;
   };
   const list=filter=>{
     const q=(filter||"").trim().toLowerCase();
     const filtered=rows.filter(x=>!q||[x.sku,x.name,x.category,x.description,x.brand].some(v=>String(v||"").toLowerCase().includes(q)));
     const g=grouped(filtered);
     document.querySelector("#ilist").innerHTML=groupHtml("PMKTOOLS 工業品","industrial",g.industrial)+groupHtml("BookWide Gift / LINE 福利品","gift",g.gifts);
     document.querySelectorAll("[data-key]").forEach(b=>b.onclick=()=>{
       document.querySelectorAll("[data-key]").forEach(x=>x.classList.toggle("active",x===b));
       const [kind,sku]=b.dataset.key.split("::");
       show(rows.find(x=>x.source_group===kind&&String(x.sku)===sku));
     });
   };

   document.querySelector("#iq").oninput=e=>list(e.target.value);
   list("");
   if(industrialRows[0])show(industrialRows[0]); else if(giftRows[0])show(giftRows[0]);

   const msg=[`PMKTOOLS 工業品 ${industrialRows.length} SKU`,`BookWide Gift / LINE ${giftRows.length} SKU`];
   if(pmkRes.status==="rejected")msg.push("工業品 API 讀取失敗");
   if(giftRes.status==="rejected")msg.push("BookWide Gift ERP Items 讀取失敗");
   ctx.status(msg.join("｜"));
 }
};
...
const salesModule={
 ...
 async render(page,ctx){
   ...
   const t=page,label=types[t];
   const [giftItemsRes,pmkItemsRes,customerRes]=await Promise.allSettled([api("/api/items"),pmkApi("/api/pmk-erp-items"),api("/api/parties?kind=customer")]);
   const giftItems=giftItemsRes.status==="fulfilled"?normalizeGiftItems(giftItemsRes.value.rows||[]):[];
   const industrialItems=pmkItemsRes.status==="fulfilled"?(pmkItemsRes.value.rows||[]).map(x=>({...x,source_group:"industrial"})):[];
   const items=[...industrialItems,...giftItems],customers=customerRes.status==="fulfilled"?(customerRes.value.rows||[]):[];
